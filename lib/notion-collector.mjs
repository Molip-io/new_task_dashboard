import { queryDatabase, searchDatabases, flatten, dbTitle, retrieveBlockChildren } from './notion.mjs';
...
import { excludePausedHierarchy, resolveTaskProjects } from './task-hierarchy.mjs';
...
export function taskFrom(row, titleProject, projectByNotionId, ignoredIds) {
  ...
  return {
    ...
    branch: single(pick(row, ['브랜치', 'Branch', 'Git 브랜치', 'GitHub 브랜치'])) || null,
...
async function collectWork(allProjects, config, errors) {
  ...
  return { tasks: excludePausedHierarchy(resolveTaskProjects([...tasks.values()])), notionSetup: setup };
}
