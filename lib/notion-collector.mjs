...
export async function collectSummaryRows(config, errors = [], since = new Date(Date.now() - 14 * 86400_000).toISOString(), { hydrateSummaryBodies = true } = {}) {
  const summaryRows = [];
  try {
    ...
    for (const page of pages) {
      ...
      if (hydrateSummaryBodies && !machinePayload && !row['분석 결과 JSON']) {
