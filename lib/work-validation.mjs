...
export function validateWorkManagement({
  ...
  tasks = excludeUncollectedHierarchy(tasks).filter(task => !DONE.has(task.status));
  ...
  const repositoryIssueTypes = {
    ...
    'not-accessible': ['GIT_AUTH_REQUIRED', 'Git 저장소가 없거나 현재 토큰에 접근 권한이 없음'],
  ...
  for (const task of tasks) {
    ...
    const sprintRequired = project?.sprintRequired !== false;
    const projectCurrentSprints = project?.sourceCurrentSprints || project?.currentSprints || [];
    const sprintRelation = sprintRequired ? classifySprint(task.sprint, projectCurrentSprints) : 'not-applicable';
    ...
    if (sprintRequired && !task.sprint) addIssue(issues, { ...base, type: 'MISSING_SPRINT', severity: 'error', message: '스프린트 없음' });
    ...
    if (isWorkItem) {
      if (!spec) addIssue(issues, { ...base, type: 'MISSING_SPEC', severity: 'error', message: '상위 작업 연결 필요', recommendedAction: '작업항목을 상위 핵심 작업에 연결하세요.' });
      if (hierarchyDepth(task, tasksById) > 1) addIssue(issues, { ...base, type: 'INVALID_HIERARCHY', severity: 'error', message: '3단계 이상의 작업 구조', recommendedAction: '상위 작업과 작업항목의 2단계 구조로 이동하세요.' });
      ...
      if (spec && task.start && spec.start && task.start < spec.start) addIssue(issues, { ...base, type: 'DATE_RANGE_MISMATCH', severity: 'warning', message: '작업항목 시작일이 상위 작업보다 빠름', recommendedAction: '상위·하위 기간을 확인하세요.' });
      if (spec && task.due && spec.due && task.due > spec.due) addIssue(issues, { ...base, type: 'DATE_RANGE_MISMATCH', severity: 'warning', message: '작업항목 마감일이 상위 작업 기간을 벗어남', recommendedAction: '상위 완료일 또는 작업항목 마감일을 조정하세요.' });
    } else {
    ...
    if (isWorkItem && task.status === '진행 예정' && task.start && task.start < today) {
      const elapsedDays = calendarDaysBetween(task.start, today);
      addIssue(issues, {
        ...base,
        type: 'PLANNED_START_DATE_PASSED',
        severity: 'warning',
        message: `시작일이 ${elapsedDays}일 지났지만 진행 예정 상태`,
        recommendedAction: '실제 착수했다면 상태를 진행 중으로 변경하고, 아직 시작하지 않았다면 시작일을 조정하세요.',
        metadata: { startDate: task.start, elapsedDays },
      });
    }
    ...
    if (task.status === START_BEFORE && sprintRelation === 'unknown' && task.sprint && Array.isArray(projectCurrentSprints)) addIssue(issues, {
      ...base,
      type: 'RULE_NOT_EVALUATED',
      severity: 'check',
      message: projectCurrentSprints.length ? '현재·미래·지난 스프린트 관계를 판정할 수 없음' : '프로젝트 현재 스프린트 설정 없음',
      metadata: { rule: 'sprint-relation', sprint: task.sprint, currentSprints: projectCurrentSprints },
    });
    ...
    if (overdueDays > 0) {
      const commentEvidence = task.delayCommentEvidence || {};
      const reasonPresent = Boolean(task.delayReason || commentEvidence.reasonPresent);
      const dateHistoryPresent = Boolean(task.previousDue || commentEvidence.dateHistoryPresent);
      const taggedUsers = new Set([
        ...(task.delayTaggedUsers || []).map(user => user?.id || user).filter(Boolean),
        ...(commentEvidence.taggedUserIds || []),
      ]);
      const uncheckedFields = [];
      if (!reasonPresent) uncheckedFields.push('delayReason');
      if (!dateHistoryPresent) uncheckedFields.push('dateHistory');
      if (!taggedUsers.size) uncheckedFields.push('ownerTag');

      if (uncheckedFields.length && task.commentCheckAvailable === false) {
        addIssue(issues, {
          ...base,
          type: 'RULE_NOT_EVALUATED',
          severity: 'check',
          message: '지연 기록 댓글을 확인하지 못해 일부 규칙을 평가할 수 없음',
          recommendedAction: '댓글 접근 상태를 확인한 뒤 지연 기록 규칙을 다시 평가하세요.',
          metadata: { rule: 'delay-record', uncheckedFields },
        });
      } else {
        if (!reasonPresent) addIssue(issues, { ...base, type: 'MISSING_DELAY_REASON', severity: 'warning', message: '지연 사유 기록 없음', recommendedAction: '상위 페이지 또는 연결된 업무 댓글에 지연 사유를 기록하세요.' });
        if (!dateHistoryPresent) addIssue(issues, { ...base, type: 'MISSING_DELAY_DATE_HISTORY', severity: 'check', message: '변경 전후 일정 기록 확인 필요', recommendedAction: '변경 전 날짜와 변경 후 날짜를 함께 기록하세요.' });
        if (!taggedUsers.size) addIssue(issues, { ...base, type: 'MISSING_DELAY_OWNER_TAG', severity: 'check', message: '지연 공유 담당자 태그 확인 필요', recommendedAction: 'PD 또는 메인 기획자를 태그하세요.' });
      }
    }
