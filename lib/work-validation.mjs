...
function addIssue(issues, issue) {
  const subject = issue.workItemId || issue.specId || issue.metadata?.commitHash || 'project';
  issues.push({
...
export function validateWorkManagement({
  ...
  for (const task of tasks) {
    ...
    if (isWorkItem && IN_PROGRESS.has(task.status) && task.edited) {
      const staleDays = businessDaysBetween(task.edited, now);
      if (staleDays >= staleBusinessDays) addIssue(issues, { ...base, type: 'STALE_UPDATE', severity: 'warning', message: `최근 ${staleDays}영업일 동안 현황 미갱신`, recommendedAction: '현재 진행 현황과 다음 액션을 갱신하세요.', metadata: { staleBusinessDays: staleDays } });
    }
  ...
  for (const commit of gitActivity.filter(commit => !commit.workItemId)) {
    const project = projectNames.has(commit.project) ? commit.project : null;
    addIssue(issues, { type: 'UNMAPPED_GIT_ACTIVITY', severity: 'check', message: project ? '작업항목을 연결할 수 없는 Git 활동' : '프로젝트를 연결할 수 없는 Git 활동', project, specId: null, workItemId: null, detectedAt, recommendedAction: project ? '커밋 메시지에 작업 키를 포함하거나 매핑 규칙을 추가하세요.' : 'Git 저장소의 프로젝트 매핑을 설정하세요.', metadata: { commitHash: commit.hash, committedAt: commit.committedAt, url: commit.url } });
  }
  ...
  const workItems = tasks.filter(task => (task.parentIds || []).length).map(task => {
    ...
    const stale = itemIssues.find(issue => issue.type === 'STALE_UPDATE')?.metadata.staleBusinessDays || 0;
    return {
      ...task,
