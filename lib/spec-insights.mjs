...
function titleTokens(value) {
  return String(value || '').toLowerCase().match(/[\p{L}\p{N}]+/gu)?.map(norm)
...
function sourceTerms(spec, frequencies) {
  return [spec.title, ...(spec.tasks || []).map(task => task.title)]
    .map(value => {
...
function slackEvidence(channels, terms) {
  return (channels || []).flatMap(channel => (channel.messages || [])
    ...
    .map(message => ({
      ...
      excerpt: excerpt([
        message.parentText ? matchedExcerpt(message.parentText, terms) : '',
        message.text,
      ].filter(Boolean).join(' → ')),
