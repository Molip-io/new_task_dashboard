...
export const ISSUE_CATALOG = Object.freeze({
  MISSING_TITLE: guide('작업명 입력 필요', WORK_ASSIGNEE, 'work-item', '작업명을 입력하세요.'),
  MISSING_SPRINT: guide('스프린트 입력 필요', WORK_ASSIGNEE, 'work-item', '작업의 스프린트를 입력하세요.'),
  MISSING_STATUS: guide('상태 입력 필요', WORK_ASSIGNEE, 'work-item', '작업 상태를 입력하세요.'),
  MISSING_DESCRIPTION: guide('작업 설명 입력 필요', PROJECT_LEAD, 'spec', '상위항목 본문에 작업 목적·범위·완료 기준을 입력하세요.'),
  MISSING_REQUIRED_OWNERS: guide('PD·팀장 담당자 지정 필요', PROJECT_LEAD, 'spec', '프로젝트 리스트의 PD와 팀장을 상위항목 담당자로 지정하세요.'),
  ...
  MISSING_PRIORITY: guide('우선순위 입력 필요', WORK_ASSIGNEE, 'work-item', '작업의 우선순위를 입력하세요.'),
  MISSING_BRANCH: guide('브랜치 입력 필요', PROJECT_DEVELOPER, 'work-item', '작업에 사용할 Git 브랜치를 입력하세요.'),
  MISSING_CONFIRMATION_COMMENT_TAG: guide('확인 요청 태그 필요', PROJECT_LEAD, 'work-item', '댓글에서 PD 또는 완료처리 담당자를 태그하세요.'),
  ...
  PLANNED_START_DATE_PASSED: guide('상태 변경 필요', WORK_ASSIGNEE, 'work-item', '실제 착수했다면 상태를 진행 중으로 변경하고, 아직 시작하지 않았다면 시작일을 조정하세요.'),
  MISSING_PROJECT: guide('프로젝트 연결 필요', PROJECT_OPERATOR, 'work-item', '요약 대상 프로젝트를 연결하세요.'),
  MISSING_SPEC: guide('상위 작업 연결 필요', PROJECT_LEAD, 'work-item', '작업항목을 상위 핵심 작업에 연결하세요.'),
  INVALID_HIERARCHY: guide('계층 수정 필요', PROJECT_LEAD, 'work-item', '상위 작업과 작업항목의 2단계 구조로 이동하세요.'),
  ...
  PARENT_CHILD_STATUS_MISMATCH: guide('상하위 상태 확인', PROJECT_LEAD, 'spec', '상위 작업과 하위 작업항목의 상태를 일치시키세요.'),
  ...
  RULE_NOT_EVALUATED: integration('규칙 대조 미실행', PROJECT_OPERATOR, 'work-item', '필요한 Notion 속성·본문·댓글 권한을 확인하세요.'),
});
...
function readiness(label, responsibleRole, actionTarget, recommendedAction) {
  return entry('readiness', label, responsibleRole, actionTarget, recommendedAction);
}
