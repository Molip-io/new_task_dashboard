...
function projectPacket(dashboard, project) {
  ...
  return {
    ...
    analysisTargets: targetRows.map(({ item, reasons }) => ({
      ...
      title: item.title, specId: item.itemLevel === 'parent' ? item.specId || item.id : item.specId,
      status: item.status, assignees: item.assignees || [], start: item.start || null, due: item.due || null,
      sprint: item.sprint || null, sprintRelation: item.sprintRelation || 'unknown', branch: item.branch || null,
