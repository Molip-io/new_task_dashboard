...
function projectSourceEvidence(project) {
  ...
  for (const insight of project.specInsights || []) {
    ...
    for (const source of ['slack', 'meeting', 'git']) {
      const matches = candidates.filter(item => item.source === source);
      const match = matches.find(item => item.attention) || matches[0];
...
function projectPacket(dashboard, project) {
  ...
  return {
    ...
    sourceEvidenceFormat: { columns: ['specId', 'source', 'timestamp', 'title', 'excerpt', 'url', 'attentionType'] },
