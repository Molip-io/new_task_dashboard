...
const PATTERNS = Object.freeze({
  ...
  qa: /(^|\s|\[)(qa|fun\s*qa|funqa)(\]|\s|$)|디렉터\s*리뷰|품질\s*검증/i,
...
function latest(events, category) {
  return events.find(event => event.categories.includes(category)) || null;
}
...
function evidenceFromEvent(event) {
  ...
  return {
    source: 'slack',
    evidenceRole: 'project_operation',
    timestamp: event.timestamp,
    title: `#${event.channel} · ${event.categories.map(category => CATEGORY_LABELS[category]).join(' · ')}`,
    excerpt: event.excerpt,
    url: event.url,
    attention: event.attention,
    attentionType: event.attentionType,
    attentionAction: event.attentionAction,
  };
}
  ...
  for (const channel of slackChannels || []) {
    for (const message of channel.messages || []) {
      ...
      collected.push({
        ...
        attentionType: attention?.type || null,
        attentionAction: attention?.action || null,
      });
    }
  }
...
export function projectOperationInsight(operations) {
  if (!operations?.evidence?.length) return null;
  const lifecycle = operations.latestLifecycleSignal;
  const data = operations.latestData;
  const summaryParts = [];
  if (lifecycle) summaryParts.push(`최근 빌드 운영 신호: ${lifecycle.excerpt}`);
  if (data && data.url !== lifecycle?.url) summaryParts.push(`최근 데이터 신호: ${data.excerpt}`);
  return {
    specId: null,
    title: '프로젝트 운영 상태',
    summary: summaryParts.join(' '),
    blockers: operations.evidence.filter(item => item.attention).map(item => item.excerpt).slice(0, 3),
    nextAction: operations.evidence.find(item => item.attention)?.attentionAction || null,
