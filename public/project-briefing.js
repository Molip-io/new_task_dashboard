...
export function resolveProjectBriefing(dashboard, project, { now = new Date() } = {}) {
  ...
  if (outdated) limits.push('이전 날짜의 분석이거나 분석 이후 입력·근거가 변경됐습니다. 현재 상태로 확정하지 마세요.');
...
export function projectBriefingHtml(dashboard, project) {
  ...
  const narrative = view.hasNarrative ? `<div class="project-briefing-hero">${axis(view.structured ? '현재 진행 요약' : '기존 통합 요약', view.currentProgress, '', 'project-briefing-axis-primary')}</div><div class="project-briefing-grid project-briefing-grid-axes">${axis('빌드·출시 현황', view.buildRelease, emptyAxis)}${axis('빌드 성과·실험 결과', view.data, '현재 전달·배포 빌드에 연결된 성과 지표가 확인되지 않았습니다.')}</div><div class="project-briefing-duo-grid"><section class="project-briefing-axis project-briefing-axis-compact"><h4>실행 병목</h4>${listHtml(view.blockers, '분석에 명시된 실행 병목이 없습니다. 출처 확인 제한이 있으면 위험 없음으로 단정하지 않습니다.')}</section><section class="project-briefing-axis project-briefing-axis-compact"><h4>다음 주요 행동</h4>${actionHtml}</section></div>` : '<p class="project-briefing-empty">프로젝트별 Agent 통합 분석이 필요합니다. 수집된 Slack 발췌를 프로젝트 전체의 결론으로 대신 표시하지 않습니다.</p>';
