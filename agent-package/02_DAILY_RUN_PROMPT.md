...
17. 각 프로젝트의 `specCatalog` 모든 행에 `specSummaries`를 1건씩 작성해. `summary`는 현재 단계와 진행 중인 일을 1~2문장으로 써. `blockers`는 선행 작업 대기, 승인·결정 대기, 후속 일정에 영향을 주는 지연, 명시된 미해결 이슈처럼 확인된 실행 병목만 최대 3건 넣어. 우선순위·기간·브랜치·담당자·설명 누락은 관리·가이드 문제이므로 `blockers`에 넣지 마. `nextAction`은 다음 업무 행동 1건이며 관리 속성 입력·보완 문구를 쓰지 말고, 근거로 다음 업무 행동을 정할 수 없으면 상태 기반 행동 또는 `null`로 써. `evidence`에는 `sourceEvidence`를 우선 사용하고 스펙명·작업항목명·ID가 직접 연결되는 Notion·Slack·회의록·GitHub 근거만 최대 6건 넣어. 직접 근거가 없는 출처는 추정해서 채우지 말고 범위 제한은 `confidenceLimits`에 남겨.
