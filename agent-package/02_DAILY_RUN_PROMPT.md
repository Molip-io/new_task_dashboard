...
1. **오늘 입력 검증**
Notion 업무현황 요약 DB(`351b4a46-5003-80ff-8b85-f772cb93da32`)에서 `run_id=rule-input:YYYY-MM-DD-morning` 페이지를 찾아. `payload` 속성의 위치 안내를 확인하고 본문을 페이지네이션 끝까지 읽어 caption `MOLIP_AGENT_INPUT_V1`인 마지막 JSON 코드 블록의 모든 조각을 결합·파싱해. 본문 `runId=YYYY-MM-DD-morning`, `outputSchema`, `rules.metrics`, `projects`, `sourceHealth`와 프로젝트별 감사 행·형식·대조 대상·스펙 목록·형식을 검증해. 입력이 없거나 불완전하면 외부 출처로 재구성하거나 요약을 저장하지 말고 failed로 종료해.
...
2. **기준 고정과 전체 범위 해석**
입력 pageId·runId·generatedAt·본문 식별 정보를 기록해. 중복 입력 내용이 달라 정본을 확정할 수 없으면 합치거나 임의 선택하지 마. 감사 행은 `ruleAuditFormat.columns`와 `indexedValues`로 먼저 복원하고 모든 요약 대상의 `ruleAuditItems`에 지침의 상태별 규칙을 적용해. `rules.metrics` 원본과 보정 `ruleMetrics`를 구분해. ID 없는 감사 행을 다른 목록에 순서로 결합하지 마.
...
5. **저장 전 검증과 수정**
당일 outputSchema의 필드·형식·제한을 검증해. 모든 프로젝트 브리핑·스펙 1:1, 입력 범위·시각, 원본/보정 집계, 근거 링크, 현재/다음 빌드 구분, 실제 빌드 KPI, 미해결 병목과 합의/제안 구분을 다시 확인해. 숫자 나열·원문 복사·출처 상태를 성과로 쓴 문장은 다시 작성해. 저장 직전 입력을 재확인하고 변경됐으면 처음 입력 기준과 이후 미반영을 제한에 명시해 partial로 처리해. 더 최신 입력으로 만든 기존 결과를 덮어쓰지 마.
