...
> 공식 지침 문서는 이 파일 하나다. 매일 실행할 때는 이 지침과 `02_DAILY_RUN_PROMPT.md`만 함께 사용한다. 다른 복사본·예시 문서는 실행 계약으로 사용하지 않는다.
...
아래 규칙은 프로젝트 브리핑을 작성할 때 앞선 일반적인 요약 규칙보다 우선한다.

- `data`는 현재 전달·배포 빌드에 연결된 성과 지표·실험 결과만 담는다. 로그 누락, 이벤트 적재 오류, 비정상 값 사용자 수, QA 점검, Notion·Slack 수집 상태, 작업 수는 성과 지표가 아니다. 성과 근거가 없거나 현재 빌드와 연결되지 않으면 반드시 `null`로 두고, 필요한 제한만 `confidenceLimits`에 쓴다.
- 데이터 오류가 실험·출시 판단을 막는다는 직접 근거가 있으면 그 영향은 `blockers`에 짧게 기록할 수 있지만, 오류 점검 내용을 `data`에 성과처럼 넣지 않는다.
- D1·D7·RV·ARPDAU·퍼널·A/B를 쓸 때는 빌드 버전, 플랫폼·국가, 관찰 기간, 코호트·실험군, 수치·단위·분모를 함께 확인한다. 조건이 빠지면 확인된 부분만 쓰고 제한을 남긴다. 단순 수치 우세를 A/B 승리로 확정하지 않는다.
- 같은 `run_id`의 규칙 입력 페이지가 여러 개면 `pageId`, 본문 `generatedAt`, 실제 규칙·프로젝트·근거 내용을 비교한다. 생성 시각만 다른 동일 본문은 최신 페이지를 기준으로 삼되, 근거·범위·규칙이 다르면 하나를 임의 선택하지 말고 입력 충돌로 실패 또는 partial을 기록한다.
- 분석 시작 후 규칙 입력이 다시 생성·갱신되면 처음 읽은 입력을 기준으로 끝까지 분석한다. 저장 직전에 입력의 `pageId`·`generatedAt`·내용 지문을 다시 확인하고 달라졌으면 새 입력을 반영했다고 주장하지 않는다. 결과의 `confidenceLimits`에 분석 기준 시각과 이후 입력 미반영을 남긴다.
- `agent-analysis:` 또는 `dashboard-snapshot:` 페이지의 `run_id` 접두사는 저장 페이지를 찾기 위한 식별자다. 결과 JSON의 `runId`는 당일 실행 ID `YYYY-MM-DD-morning`을 유지하고, 접두사를 붙이거나 서로 다른 실행 ID로 바꾸지 않는다.
- 같은 날짜의 분석·스냅샷 페이지가 여러 개면 임의로 첫 행을 고르지 않는다. 내용·생성 시각·실행 ID를 비교해 기준 페이지를 결정하고, 결정할 수 없으면 최신성·동기화 상태를 `partial`로 남긴다. 중복 페이지를 임의 삭제하지 않는다.
- 분석 결과가 저장된 뒤 더 최신의 규칙 입력·근거가 생겼으면 이전 결과를 현재 결과로 위장하지 않는다. `분석 시각`과 `입력 기준 시각`을 함께 표시하고 다음 실행에서 갱신 대상으로 남긴다.

저장 전 다음 문장을 기준으로 자체 점검한다. “이 값은 현재 빌드의 성과인가, 아니면 데이터가 제대로 수집됐는지에 대한 상태인가?” 후자면 `data`에서 제거한다. “이 분석이 어느 입력 페이지를 기준으로 했는가?”를 pageId·generatedAt·실행 ID로 재현할 수 없으면 저장을 성공으로 보고하지 않는다.
