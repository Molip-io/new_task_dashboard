...
try:
    with sync_playwright() as p:
        ...
        page.locator('.project-briefing-card > summary').first.click()
        check('stale analysis stays explicitly labelled', '이전 통합 분석 · 갱신 필요' in page.locator('.project-briefing-meta').first.inner_text())
        d=copy.deepcopy(fixture);d['ai']['sourceStatus']['slack']='failed';render(d)
        page.locator('.project-briefing-card > summary').first.click()
        check('partial source coverage stays explicit','확인 제한' in page.locator('.project-briefing-meta').first.inner_text() and 'Slack failed' in page.locator('.project-briefing-limits').first.inner_text())
